...
Hivemind requires a postgres backend. By default, `confpg/10/main/conf.d/custom.conf` contains a PostgreSQL configuration which is tuned to make use
of up to 16GB of RAM, and 8 cores.

For the best Hivemind performance, you should generate a tuned PostgreSQL configuration, based on your system's specifications [using PGTune](https://pgtune.leopard.in.ua).

Open up `confpg/10/main/conf.d/custom.conf` in your preferred editor, remove the existing configuration lines, and paste in the config lines
which were generated by [PGTune](https://pgtune.leopard.in.ua)

```sh
nano confpg/10/main/conf.d/custom.conf
```

Once you've adjusted the PostgreSQL config as needed, start postgres with these two commands consecutively:
